...
public class FreestyleJobPublisherTest {
    ...
    public void testFreestyleWithFalsePublisher() throws Exception {
        ...
        FreeStyleBuild b = getB15502(p); // CAP AL
 ...
 // CAP AL
    private FreeStyleBuild getB15502(final FreeStyleProject p) throws ExecutionException, InterruptedException { // CAP AL
        p.getPublishersList().add(new ResultWriterPublisher("result.txt")); //catch result to file // CAP AL
        final ArtifactArchiver artifactArchiver = new ArtifactArchiver("result.txt"); // CAP AL
        artifactArchiver.setOnlyIfSuccessful(false); // CAP AL
        p.getPublishersList().add(artifactArchiver); // transfer file to build dir // CAP AL
         // CAP AL
        FreeStyleBuild b = p.scheduleBuild2(0).get(); // CAP AL
         // CAP AL
        assertEquals("Build must fail, because we used FalsePublisher", Result.FAILURE, b.getResult()); // CAP AL
        return b; // CAP AL
    } // CAP AL
}
