...
public class EnvVarsInConfigTasksTest extends HudsonTestCase {
	...
	public void testFreeStyleAntOnSlave() throws Exception {
		...
		FreeStyleProject project = getProject6612(); // CAP AL
		...
		// test the regular agent - variable not expanded
		FreeStyleBuild build = getBuild56839(project);
 ...
 // CAP AL
	private FreeStyleProject getProject6612() throws IOException { // CAP AL
	    FreeStyleProject project = createFreeStyleProject(); // CAP AL
	    project.setJDK(jenkins.getJDK("varJDK")); // CAP AL
	    project.setScm(new ExtractResourceSCM(getClass().getResource( // CAP AL
	    		"/simple-projects.zip"))); // CAP AL
	    return project; // CAP AL
	} // CAP AL

	private FreeStyleBuild getBuild56839(final FreeStyleProject project) throws Exception {
	    project.setAssignedLabel(slaveRegular.getSelfLabel());
	    FreeStyleBuild build = project.scheduleBuild2(0).get();
	    System.out.println(build.getDisplayName() + " completed");
